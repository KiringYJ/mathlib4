...
`μ[stoppedValue f τ | hσ.sigmaAlgebra]`.
...
variable {Ω E : Type*} {m : SigmaAlgebra Ω} {μ : Measure Ω} [NormedAddCommGroup E]
...
theorem condExp_stopping_time_ae_eq_restrict_eq_const (h : Martingale f ℱ μ)
    (hτ : IsStoppingTime ℱ τ) [SigmaFinite (μ.trim hτ.sigmaAlgebra_le)] (hin : i ≤ n) :
    μ[f n | hτ.sigmaAlgebra] =ᵐ[μ.restrict {x | τ x = i}] f i := by
...
theorem condExp_stopping_time_ae_eq_restrict_eq_const_of_le_const (h : Martingale f ℱ μ)
    ...
    [SigmaFinite (μ.trim (hτ.sigmaAlgebra_le_of_le hτ_le))] (i : ι) :
    μ[f n | hτ.sigmaAlgebra] =ᵐ[μ.restrict {x | τ x = i}] f i := by
  by_cases hin : i ≤ n
  · refine Filter.EventuallyEq.trans ?_ (ae_restrict_of_ae (h.condExp_ae_eq hin))
    refine condExp_ae_eq_restrict_of_sigmaAlgebra_eq_on (hτ.sigmaAlgebra_le_of_le hτ_le)
...
theorem stoppedValue_ae_eq_restrict_eq (h : Martingale f ℱ μ) (hτ : IsStoppingTime ℱ τ)
    (hτ_le : ∀ x, τ x ≤ n) [SigmaFinite (μ.trim (hτ.sigmaAlgebra_le_of_le hτ_le))] (i : ι) :
    stoppedValue f τ =ᵐ[μ.restrict {x | τ x = i}] μ[f n | hτ.sigmaAlgebra] := by
  refine Filter.EventuallyEq.trans ?_
    (condExp_stopping_time_ae_eq_restrict_eq_const_of_le_const h hτ hτ_le i).symm
  rw [Filter.EventuallyEq, ae_restrict_iff' (ℱ.le _ (hτ.measurableSet_eq i))]
...
theorem stoppedValue_ae_eq_condExp_of_le_const_of_countable_range (h : Martingale f ℱ μ)
    ...
    [SigmaFinite (μ.trim (hτ.sigmaAlgebra_le_of_le hτ_le))] :
    stoppedValue f τ =ᵐ[μ] μ[f n | hτ.sigmaAlgebra] := by
...
theorem stoppedValue_ae_eq_condExp_of_le_const [Countable ι] (h : Martingale f ℱ μ)
    (hτ : IsStoppingTime ℱ τ) (hτ_le : ∀ x, τ x ≤ n)
    [SigmaFinite (μ.trim (hτ.sigmaAlgebra_le_of_le hτ_le))] :
    stoppedValue f τ =ᵐ[μ] μ[f n | hτ.sigmaAlgebra] :=
...
theorem stoppedValue_ae_eq_condExp_of_le_of_countable_range (h : Martingale f ℱ μ)
    ...
    [SigmaFinite (μ.trim (hσ.sigmaAlgebra_le_of_le fun x => (hσ_le_τ x).trans (hτ_le x)))] :
    stoppedValue f σ =ᵐ[μ] μ[stoppedValue f τ | hσ.sigmaAlgebra] := by
  have : SigmaFinite (μ.trim (hτ.sigmaAlgebra_le_of_le hτ_le)) :=
    sigmaFiniteTrim_mono _ (IsStoppingTime.sigmaAlgebra_mono hσ hτ hσ_le_τ)
  have : μ[stoppedValue f τ | hσ.sigmaAlgebra] =ᵐ[μ]
      μ[μ[f n | hτ.sigmaAlgebra] | hσ.sigmaAlgebra] := condExp_congr_ae
    (h.stoppedValue_ae_eq_condExp_of_le_const_of_countable_range hτ hτ_le hτ_countable_range)
  refine (Filter.EventuallyEq.trans ?_
    (condExp_condExp_of_le ?_ (hτ.sigmaAlgebra_le_of_le hτ_le)).symm).trans this.symm
  · exact h.stoppedValue_ae_eq_condExp_of_le_const_of_countable_range hσ
      (fun x => (hσ_le_τ x).trans (hτ_le x)) hσ_countable_range
  · exact hσ.sigmaAlgebra_mono hτ hσ_le_τ
...
theorem stoppedValue_ae_eq_condExp_of_le [Countable ι] (h : Martingale f ℱ μ)
    (hτ : IsStoppingTime ℱ τ) (hσ : IsStoppingTime ℱ σ) (hσ_le_τ : σ ≤ τ) (hτ_le : ∀ x, τ x ≤ n)
    [SigmaFinite (μ.trim hσ.sigmaAlgebra_le)] :
    stoppedValue f σ =ᵐ[μ] μ[stoppedValue f τ | hσ.sigmaAlgebra] :=
...
variable {ι : Type*} [LinearOrder ι] [LocallyFiniteOrder ι] [OrderBot ι] [TopologicalSpace ι]
  [DiscreteTopology ι] [SigmaAlgebra ι] [BorelSpace ι] [SigmaAlgebra E] [BorelSpace E]
...
theorem condExp_stoppedValue_stopping_time_ae_eq_restrict_le (h : Martingale f ℱ μ)
    (hτ : IsStoppingTime ℱ τ) (hσ : IsStoppingTime ℱ σ) [SigmaFinite (μ.trim hσ.sigmaAlgebra_le)]
    (hτ_le : ∀ x, τ x ≤ i) :
    μ[stoppedValue f τ | hσ.sigmaAlgebra] =ᵐ[μ.restrict {x : Ω | τ x ≤ σ x}]
      stoppedValue f τ := by
  rw [ae_eq_restrict_iff_indicator_ae_eq
    (hτ.sigmaAlgebra_le (hτ.measurableSet_le_stopping_time hσ))]
  ...
  have h_int :
    ...
    exact hτ.sigmaAlgebra_le (hτ.measurableSet_le_stopping_time hσ)
  have h_meas : AEStronglyMeasurable[hσ.sigmaAlgebra]
      ({ω : Ω | τ ω ≤ σ ω}.indicator (stoppedValue (fun n : ι => f n) τ)) μ := by
    refine StronglyMeasurable.aestronglyMeasurable ?_
    refine StronglyMeasurable.stronglyMeasurable_of_sigmaAlgebra_le_on
  ...
  exact condExp_of_aestronglyMeasurable' hσ.sigmaAlgebra_le h_meas h_int
...
theorem stoppedValue_min_ae_eq_condExp [SigmaFiniteFiltration μ ℱ] (h : Martingale f ℱ μ)
    ...
    [h_sf_min : SigmaFinite (μ.trim (hτ.min hσ).sigmaAlgebra_le)] :
    (stoppedValue f fun x => min (σ x) (τ x)) =ᵐ[μ] μ[stoppedValue f τ | hσ.sigmaAlgebra] := by
  refine
  ...
  · suffices μ[stoppedValue f τ | (hσ.min hτ).sigmaAlgebra] =ᵐ[μ.restrict {x | τ x ≤ σ x}]
    ...
    · rw [IsStoppingTime.sigmaAlgebra_min hσ hτ,
        IsStoppingTime.sigmaAlgebra_min hτ hσ, inf_comm]
    · have h1 : μ[stoppedValue f τ | hτ.sigmaAlgebra] = stoppedValue f τ := by
        apply condExp_of_stronglyMeasurable hτ.sigmaAlgebra_le
