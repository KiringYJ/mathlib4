...
if for any `A ∈ m`, any `I : Finset S` and any bounded continuous
...
variable {Ω S T : Type*} {m mΩ : SigmaAlgebra Ω} {P : Measure Ω}
...
lemma IndepFun.singleton_indepSets_of_indicator {𝓧 : Type*} [mX : SigmaAlgebra 𝓧] {A : Set Ω}
    {X : Ω → 𝓧} (h : (A.indicator (1 : Ω → ℝ)) ⟂ᵢ[P] X) :
    IndepSets {A} (mX.comap X : Set (Set Ω)) P := by
  rw [IndepSets_iff]
...
variable {E : S → Type*} {F : T → Type*} {G H : Type*}
  [∀ s, TopologicalSpace (E s)] [∀ s, SigmaAlgebra (E s)] [∀ s, BorelSpace (E s)]
  [∀ s, HasOuterApproxClosed (E s)]
  [∀ t, TopologicalSpace (F t)] [∀ t, SigmaAlgebra (F t)] [∀ t, BorelSpace (F t)]
  [∀ t, HasOuterApproxClosed (F t)]
  [TopologicalSpace G] [SigmaAlgebra G] [BorelSpace G] [HasOuterApproxClosed G]
  [TopologicalSpace H] [SigmaAlgebra H] [BorelSpace H] [HasOuterApproxClosed H]
...
lemma indepSets_comap_pi_of_prod_bcf
    ...
    (h : ∀ A ∈ 𝒜, ∀ f : (s : S) → E s →ᵇ ℝ, ∫ ω in A, ∏ s, f s (X s ω) ∂P =
      P.real A * ∫ ω, ∏ s, f s (X s ω) ∂P) :
    IndepSets 𝒜 (SigmaAlgebra.pi.comap (fun ω s ↦ X s ω) : Set (Set Ω)) P :=
...
lemma indepSets_comap_of_bcf
    ...
    IndepSets 𝒜 (SigmaAlgebra.comap Z inferInstance : Set (Set Ω)) P :=
  indepSets_iff_singleton_indepSets.2 fun A hA ↦ IndepFun.singleton_indepSets_of_indicator
...
lemma indep_comap_pi_of_prod_bcf (hm : m ≤ mΩ) (mX : ∀ s, AEMeasurable (X s) P)
    ...
    Indep m (SigmaAlgebra.pi.comap (fun ω s ↦ X s ω)) P :=
  (Indep_iff_IndepSets _ _ P).2
    (indepSets_comap_pi_of_prod_bcf
      (fun _ hA ↦ MeasurableSet.nullMeasurableSet (hm hA)) mX h)
...
lemma indep_comap_of_bcf (hm : m ≤ mΩ) (mZ : AEMeasurable Z P)
    ...
    Indep m (SigmaAlgebra.comap Z inferInstance) P :=
  (Indep_iff_IndepSets _ _ P).2
    (indepSets_comap_of_bcf
      (fun _ hA ↦ MeasurableSet.nullMeasurableSet (hm hA)) mZ h)
...
lemma indep_comap_process_of_prod_bcf
    ...
    (h : ∀ A ∈ m, ∀ (I : Finset S) (f : (s : I) → E s →ᵇ ℝ),
      ∫ ω in A, ∏ s, f s (X s ω) ∂P = P.real A * ∫ ω, ∏ s, f s (X s ω) ∂P) :
    Indep m (SigmaAlgebra.pi.comap (fun ω s ↦ X s ω)) P :=
  (Indep_iff_IndepSets _ _ P).2
...
lemma indep_comap_process_of_bcf
    (hm : m ≤ mΩ) (mX : ∀ s, AEMeasurable (X s) P)
    (h : ∀ A ∈ m, ∀ (I : Finset S) (f : (Π s : I, E s) →ᵇ ℝ),
      ∫ ω in A, f (X · ω) ∂P = P.real A * ∫ ω, f (X · ω) ∂P) :
    Indep m (SigmaAlgebra.pi.comap (fun ω s ↦ X s ω)) P :=
  (Indep_iff_IndepSets _ _ P).2
