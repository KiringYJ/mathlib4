...
variable {Ω 𝓧 : Type*} {mΩ : SigmaAlgebra Ω} {m𝓧 : SigmaAlgebra 𝓧} {X Y : Ω → 𝓧}
...
lemma HasLaw.comp_of_hasLaw_comp {Ω' 𝓨 : Type*} {m' : SigmaAlgebra Ω'} {m𝓨 : SigmaAlgebra 𝓨}
...
lemma _root_.MeasureTheory.MeasurePreserving.comp_hasLaw {𝓨 : Type*} {m𝓨 : SigmaAlgebra 𝓨}
    {ν : Measure 𝓨} {Y : 𝓧 → 𝓨} (hY : MeasurePreserving Y μ ν) (hX : HasLaw X μ P) :
    HasLaw (Y ∘ X) ν P :=
  hY.hasLaw.comp hX

lemma _root_.MeasureTheory.MeasurePreserving.fun_comp_hasLaw {𝓨 : Type*} {m𝓨 : SigmaAlgebra 𝓨}
    {ν : Measure 𝓨} {Y : 𝓧 → 𝓨} (hY : MeasurePreserving Y μ ν) (hX : HasLaw X μ P) :
    HasLaw (fun ω ↦ Y (X ω)) ν P :=
  hY.comp_hasLaw hX

@[to_additive]
lemma IndepFun.hasLaw_mul {M : Type*} [Monoid M] {mM : SigmaAlgebra M} [MeasurableMul₂ M]
...
lemma iIndepFun.hasLaw_pi {ι : Type*} [Fintype ι] {𝓧 : ι → Type*} {m𝓧 : ∀ i, SigmaAlgebra (𝓧 i)}
