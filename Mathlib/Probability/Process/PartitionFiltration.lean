...
partitions.
This sequence of partitions is defined in `MeasureTheory.SigmaAlgebra.CountablyGenerated`.
...
variable {α : Type*} [m : SigmaAlgebra α] {t : ℕ → Set α}
...
lemma measurable_memPartitionSet_subtype (ht : ∀ n, MeasurableSet (t n)) (n : ℕ)
    (m : SigmaAlgebra (memPartition t n)) :
...
variable {α : Type*} [SigmaAlgebra α] [CountablyGenerated α]

/-- A filtration built from the measurable spaces generated by `countablePartition α n` for
all `n : ℕ`. -/
-- Note: `Set` has no computational content, but Lean still attempts to compile it.
-- See https://github.com/leanprover/lean4/issues/14084.
noncomputable def countableFiltration (α : Type*) [m : SigmaAlgebra α] [CountablyGenerated α] :
    Filtration ℕ m where
...
lemma measurable_countablePartitionSet_subtype (n : ℕ)
    (m : SigmaAlgebra (countablePartition α n)) :
...
lemma measurable_countableFiltration_countablePartitionSet (α : Type*)
    [SigmaAlgebra α] [CountablyGenerated α] (n : ℕ) :
...
lemma measurable_countablePartitionSet (α : Type*) [SigmaAlgebra α] [CountablyGenerated α]
    (n : ℕ) :
...
lemma iSup_countableFiltration (α : Type*) [m : SigmaAlgebra α] [CountablyGenerated α] :
