...
bounded by `(max #s 2) ^ ℵ₀`. This is stated in `SigmaAlgebra.cardinal_generate_measurable_le`
and `SigmaAlgebra.cardinalMeasurableSet_le`.

In particular, if `#s ≤ 𝔠`, then the generated sigma-algebra has cardinality at most `𝔠`, see
`SigmaAlgebra.cardinal_measurableSet_le_continuum`.
...
each step of the construction. We show in `SigmaAlgebra.generateMeasurable_eq_rec` that this
...
theorem cardinal_measurableSet_le (s : Set (Set α)) :
    #(generateFrom s : Set (Set α)) ≤ max #s 2 ^ ℵ₀ :=
...
theorem cardinal_measurableSet_le_continuum {s : Set (Set α)} :
    #s ≤ 𝔠 → #(generateFrom s : Set (Set α)) ≤ 𝔠 :=
