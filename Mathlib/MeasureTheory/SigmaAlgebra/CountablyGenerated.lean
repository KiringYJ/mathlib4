...
* `SigmaAlgebra.CountablyGenerated`: class stating that a measurable space is countably
  generated.
* `SigmaAlgebra.countableGeneratingSet`: a countable set of sets that generates the σ-algebra.
* `SigmaAlgebra.countablePartition`: sequences of finer and finer partitions of
  a countably generated space, defined by taking the `memPartition` of an enumeration of the sets in
  `countableGeneratingSet`.
* `SigmaAlgebra.SeparatesPoints` : class stating that a measurable space separates points.

## Main statements

* `SigmaAlgebra.measurableEquiv_nat_bool_of_countablyGenerated`: if a measurable space is
...
* `SigmaAlgebra.measurable_injection_nat_bool_of_countablySeparated`: If a measurable space
...
class CountablyGenerated (α : Type*) [m : SigmaAlgebra α] : Prop where
...
noncomputable
def countableGeneratingSet (α : Type*) [SigmaAlgebra α] [h : CountablyGenerated α] :
    Set (Set α) :=
  insert ∅ h.isCountablyGenerated.choose

lemma countable_countableGeneratingSet [SigmaAlgebra α] [h : CountablyGenerated α] :
    Set.Countable (countableGeneratingSet α) :=
  Countable.insert _ h.isCountablyGenerated.choose_spec.1

lemma generateFrom_countableGeneratingSet [m : SigmaAlgebra α] [h : CountablyGenerated α] :
    generateFrom (countableGeneratingSet α) = m :=
  (generateFrom_insert_empty _).trans <| h.isCountablyGenerated.choose_spec.2.symm

lemma empty_mem_countableGeneratingSet [SigmaAlgebra α] [CountablyGenerated α] :
    ∅ ∈ countableGeneratingSet α := mem_insert _ _

lemma nonempty_countableGeneratingSet [SigmaAlgebra α] [CountablyGenerated α] :
    Set.Nonempty (countableGeneratingSet α) :=
  ⟨∅, mem_insert _ _⟩

lemma measurableSet_countableGeneratingSet [SigmaAlgebra α] [CountablyGenerated α]
    {s : Set α} (hs : s ∈ countableGeneratingSet α) :
    MeasurableSet s := by
  rw [← generateFrom_countableGeneratingSet (α := α)]
  exact mem_generateFrom hs

/-- A countable sequence of sets generating the measurable space. -/
-- Note: `Set` has no computational content, but Lean still attempts to compile it.
-- See https://github.com/leanprover/lean4/issues/14084.
noncomputable
def natGeneratingSequence (α : Type*) [SigmaAlgebra α] [CountablyGenerated α] : ℕ → (Set α) :=
  enumerateCountable (countable_countableGeneratingSet (α := α)) ∅

lemma generateFrom_natGeneratingSequence (α : Type*) [m : SigmaAlgebra α]
...
lemma measurableSet_natGeneratingSequence [SigmaAlgebra α] [CountablyGenerated α] (n : ℕ) :
...
theorem CountablyGenerated.comap [m : SigmaAlgebra β] [h : CountablyGenerated β] (f : α → β) :
...
theorem CountablyGenerated.sup {m₁ m₂ : SigmaAlgebra β} (h₁ : @CountablyGenerated β m₁)
...
instance (priority := 100) [SigmaAlgebra α] [Countable α] : CountablyGenerated α where
  isCountablyGenerated := by
    let m := (inferInstance : SigmaAlgebra α)
    refine ⟨⋃ y, {m.indistinguishabilityClass y},
      countable_iUnion (fun i ↦ countable_singleton _), ?_⟩
    apply le_antisymm
    · intro s hs
      have : s = ⋃ y ∈ s, m.indistinguishabilityClass y := by
        apply Subset.antisymm
        · intro x hx
          exact mem_iUnion_of_mem x <| mem_iUnion_of_mem hx <|
            m.self_mem_indistinguishabilityClass x
        · simp only [iUnion_subset_iff]
          intro x hx
          exact m.indistinguishabilityClass_subset hs hx
      rw [this]
      apply MeasurableSet.biUnion (to_countable s)
      intro x _hx
      exact mem_generateFrom <| mem_iUnion_of_mem x <| by simp
    · apply generateFrom_le
      intro s hs
      simp only [mem_iUnion, mem_singleton_iff] at hs
      obtain ⟨x, rfl⟩ := hs
      exact m.indistinguishabilityClass_mem_of_countable x

instance [SigmaAlgebra α] [CountablyGenerated α] {p : α → Prop} :
    CountablyGenerated { x // p x } := .comap _

instance [SigmaAlgebra α] [CountablyGenerated α] [SigmaAlgebra β] [CountablyGenerated β] :
    CountablyGenerated (α × β) :=
  .sup (.comap Prod.fst) (.comap Prod.snd)

section GeneratorMembershipClass

variable [mα : SigmaAlgebra α] [CountablyGenerated α]

open scoped Classical in
/-- The membership classes determined by a countable generating sequence.

Some classes may be empty. The class associated to the membership pattern of a point is its
indistinguishability class; see
`indistinguishabilityClass_eq_generatorMembershipClass_natGeneratingSequence`. -/
-- Note: `Set` has no computational content, but Lean still attempts to compile it.
-- See https://github.com/leanprover/lean4/issues/14084.
noncomputable
def generatorMembershipClass (α : Type*) [SigmaAlgebra α] [CountablyGenerated α] :
    (ℕ → Prop) → Set α :=
  fun p ↦ ⋂ n, if p n then natGeneratingSequence α n else (natGeneratingSequence α n)ᶜ

lemma generatorMembershipClass_mem (p : ℕ → Prop) :
    generatorMembershipClass α p ∈ mα := by
...
lemma disjoint_generatorMembershipClass :
    Pairwise (Function.onFun Disjoint (generatorMembershipClass α)) := by
  ...
  specialize hsp hxs
  specialize hsq hxs
  simp only [generatorMembershipClass, mem_iInter] at hsp hsq
  grind

lemma iUnion_generatorMembershipClass : ⋃ p, generatorMembershipClass α p = univ := by
  ext x
  simp only [generatorMembershipClass, mem_iUnion, mem_iInter, mem_univ, iff_true]
  exact ⟨fun n ↦ x ∈ natGeneratingSequence α n, by grind⟩

lemma mem_generatorMembershipClass_natGeneratingSequence (x : α) :
    x ∈ generatorMembershipClass α (x ∈ natGeneratingSequence α ·) := by
  simp [generatorMembershipClass]; grind

open scoped Classical in
/-- Any measurable set in a countably generated sigma-algebra is a union of generator-membership
classes. -/
lemma exists_eq_iUnion_generatorMembershipClass {s : Set α} (hs : MeasurableSet s) :
    ∃ (q : (ℕ → Prop) → Prop), s = ⋃ p, if q p then generatorMembershipClass α p else ∅ := by
  rw [← generateFrom_natGeneratingSequence α] at hs
  refine generateFrom_induction (range (natGeneratingSequence α))
    (fun s _ ↦ ∃ (q : (ℕ → Prop) → Prop),
      s = ⋃ p, if q p then generatorMembershipClass α p else ∅) ?_ ?_ ?_ ?_ hs
  · simp only [mem_range, forall_exists_index, forall_apply_eq_imp_iff]
    refine fun n ↦ ⟨fun p ↦ p n, ?_⟩
    ext x
    simp only [mem_iUnion, mem_ite_empty_right]
    refine ⟨fun hx ↦ ?_, ?_⟩
    · exact ⟨(x ∈ natGeneratingSequence α ·), by simp [generatorMembershipClass]; grind⟩
    · rintro ⟨p, hpn, hpx⟩
      simp only [generatorMembershipClass, mem_iInter] at hpx
      specialize hpx n
      simpa [hpn] using hpx
  ...
  · simp only [forall_exists_index]
    ...
    · refine ⟨fun n ↦ x ∈ natGeneratingSequence α n, ?_,
        mem_generatorMembershipClass_natGeneratingSequence x⟩
      contrapose! h
      refine ⟨fun n ↦ x ∈ natGeneratingSequence α n, h,
        mem_generatorMembershipClass_natGeneratingSequence x⟩
    · rintro ⟨p, hpq, hpx⟩ p' hqp' hx_mem
      have hpp' : p ≠ p' := by grind
      have h_disj : Disjoint (generatorMembershipClass α p) (generatorMembershipClass α p') :=
        disjoint_generatorMembershipClass hpp'
      grind
  · intro t ht h
    choose q hq using h
...
/-- The indistinguishability class of a point in a countably generated sigma-algebra is determined
by its membership pattern on a countable generating sequence. -/
lemma indistinguishabilityClass_eq_generatorMembershipClass_natGeneratingSequence (x : α) :
    mα.indistinguishabilityClass x =
      generatorMembershipClass α (x ∈ natGeneratingSequence α ·) := by
  ...
  have hgen : (∀ s ∈ mα, x ∈ s ↔ y ∈ s) ↔ ∀ s ∈ range A, x ∈ s ↔ y ∈ s := by
    rw [← forall_generateFrom_mem_iff_mem_iff]
    simp only [hA_gen]
  rw [hgen]
  simp only [mem_range, forall_exists_index, forall_apply_eq_imp_iff, generatorMembershipClass,
    mem_iInter]
  grind

/-- In a countably generated sigma-algebra, every indistinguishability class is measurable. -/
lemma indistinguishabilityClass_mem (x : α) :
    mα.indistinguishabilityClass x ∈ mα := by
  rw [indistinguishabilityClass_eq_generatorMembershipClass_natGeneratingSequence]
  exact generatorMembershipClass_mem _

end GeneratorMembershipClass
...
class SeparatesPoints (α : Type*) [m : SigmaAlgebra α] : Prop where
  separates : ∀ x y : α, (∀ s, MeasurableSet s → (x ∈ s → y ∈ s)) → x = y

theorem separatesPoints_def [SigmaAlgebra α] [hs : SeparatesPoints α] {x y : α}
    (h : ∀ s, MeasurableSet s → (x ∈ s → y ∈ s)) : x = y := hs.separates _ _ h

theorem exists_measurableSet_of_ne [SigmaAlgebra α] [SeparatesPoints α] {x y : α}
    (h : x ≠ y) : ∃ s, MeasurableSet s ∧ x ∈ s ∧ y ∉ s := by
  contrapose! h
  exact separatesPoints_def h

theorem separatesPoints_iff [SigmaAlgebra α] : SeparatesPoints α ↔
...
theorem SeparatesPoints.mono {m m' : SigmaAlgebra α} [hsep : @SeparatesPoints _ m] (h : m ≤ m') :
    @SeparatesPoints _ m' := @SeparatesPoints.mk _ m' fun _ _ hxy ↦
    @SeparatesPoints.separates _ m hsep _ _ fun _ hs ↦ hxy _ (h hs)

theorem _root_.eq_const_of_measurable_bot [SigmaAlgebra β] [Nonempty β]
    [SeparatesPoints β] {f : α → β} (hf : Measurable[⊥] f) :
    ∃ c, f = fun _ ↦ c := by
  have h (a₁ : α) (a₂ : α) : f a₁ = f a₂ := by
    by_contra! h
    obtain ⟨s, hs, hx, hy⟩ := exists_measurableSet_of_ne h
    obtain h' | h' := SigmaAlgebra.mem_bot_iff.mp (hf hs)
    · absurd hx
      simp [← mem_preimage, h']
    · absurd hy
      simp [← mem_preimage, h']
...
instance (priority := 100) Subtype.separatesPoints [SigmaAlgebra α] [h : SeparatesPoints α]
    {s : Set α} : SeparatesPoints s :=
  ⟨fun _ _ hxy ↦ Subtype.val_injective <| h.1 _ _ fun _ ht ↦ hxy _ <| measurable_subtype_coe ht⟩

instance (priority := 100) Subtype.countablySeparated [SigmaAlgebra α]
...
instance (priority := 100) separatesPoints_of_measurableSingletonClass [SigmaAlgebra α]
...
instance (priority := 50) MeasurableSingletonClass.of_separatesPoints [SigmaAlgebra α]
...
instance hasCountableSeparatingOn_of_countablySeparated_subtype
    [SigmaAlgebra α] {s : Set α} [h : CountablySeparated s] :
...
instance countablySeparated_subtype_of_hasCountableSeparatingOn
    [SigmaAlgebra α] {s : Set α} [h : HasCountableSeparatingOn _ MeasurableSet s] :
    CountablySeparated s := CountablySeparated.subtype_iff.mpr h

instance countablySeparated_of_separatesPoints [SigmaAlgebra α]
...
theorem exists_countablyGenerated_le_of_countablySeparated [m : SigmaAlgebra α]
    [h : CountablySeparated α] :
    ∃ m' : SigmaAlgebra α, @CountablyGenerated _ m' ∧ @SeparatesPoints _ m' ∧ m' ≤ m := by
  rcases h with ⟨b, bct, hbm, hb⟩
  refine ⟨generateFrom b, ?_, ?_, generateFrom_le hbm⟩
  · use b
  rw [@separatesPoints_iff]
  exact fun x y hxy ↦ hb _ trivial _ trivial fun _ hs ↦ hxy _ <| mem_generateFrom hs
...
noncomputable
def mapNatBool [SigmaAlgebra α] [CountablyGenerated α] (x : α) (n : ℕ) :
    Bool := x ∈ natGeneratingSequence α n

theorem measurable_mapNatBool [SigmaAlgebra α] [CountablyGenerated α] :
...
theorem injective_mapNatBool [SigmaAlgebra α] [CountablyGenerated α]
...
the Cantor Space. -/
theorem measurableEquiv_nat_bool_of_countablyGenerated [SigmaAlgebra α]
  ...
  refine ⟨{y | y n}, ?_, ?_⟩
  · simpa only [preimage, mem_singleton_iff] using
...
theorem measurable_injection_nat_bool_of_countablySeparated [SigmaAlgebra α]
...
theorem measurableSingletonClass_of_countablySeparated
    [SigmaAlgebra α] [CountablySeparated α] :
...
lemma generateFrom_iUnion_memPartition (t : ℕ → Set α) :
  ...
  · simp only [mem_iUnion] at hu
    ...
    | succ n ih =>
      ...
      · exact (generateFrom (range t)).inter_mem (ih v hv) (mem_generateFrom ⟨n, rfl⟩)
...
lemma generateFrom_iUnion_memPartition_le [m : SigmaAlgebra α] {t : ℕ → Set α}
...
lemma generateFrom_memPartition_le [m : SigmaAlgebra α] {t : ℕ → Set α}
...
lemma measurableSet_memPartition [SigmaAlgebra α] {t : ℕ → Set α}
    (ht : ∀ n, MeasurableSet (t n)) (n : ℕ) {s : Set α} (hs : s ∈ memPartition t n) :
    MeasurableSet s :=
  generateFrom_memPartition_le ht n (mem_generateFrom hs)

lemma measurableSet_memPartitionSet [SigmaAlgebra α] {t : ℕ → Set α}
...
variable [m : SigmaAlgebra α] [h : CountablyGenerated α]
...
-- Note: `Set` has no computational content, but Lean still attempts to compile it.
-- See https://github.com/leanprover/lean4/issues/14084.
noncomputable def countablePartition (α : Type*) [SigmaAlgebra α] [CountablyGenerated α] :
    ℕ → Set (Set α) :=
...
lemma measurableSet_enumerateCountable_countableGeneratingSet
    (α : Type*) [SigmaAlgebra α] [CountablyGenerated α] (n : ℕ) :
...
lemma finite_countablePartition (α : Type*) [SigmaAlgebra α] [CountablyGenerated α] (n : ℕ) :
...
lemma sUnion_countablePartition (α : Type*) [SigmaAlgebra α] [CountablyGenerated α] (n : ℕ) :
...
lemma generateFrom_iUnion_countablePartition (α : Type*) [m : SigmaAlgebra α]
...
lemma generateFrom_countablePartition_le (α : Type*) [m : SigmaAlgebra α] [CountablyGenerated α]
    (n : ℕ) :
...
lemma measurableSet_countablePartition (n : ℕ) {s : Set α} (hs : s ∈ countablePartition α n) :
    MeasurableSet s :=
  generateFrom_countablePartition_le α n (mem_generateFrom hs)
...
variable {α γ : Type*} [SigmaAlgebra γ]

/-- A class registering that either `α` is countable or `β` is a countably generated
measurable space. -/
class CountableOrCountablyGenerated (α β : Type*) [SigmaAlgebra β] :
    Prop where
  countableOrCountablyGenerated : Countable α ∨ SigmaAlgebra.CountablyGenerated β

instance instCountableOrCountablyGeneratedOfCountable [h1 : Countable α] [SigmaAlgebra β] :
    CountableOrCountablyGenerated α β := ⟨Or.inl h1⟩

instance instCountableOrCountablyGeneratedOfCountablyGenerated [SigmaAlgebra β]
