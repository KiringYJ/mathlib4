...
@[simp] theorem generateFrom_singleton (s : Set α) :
    generateFrom {s} = SigmaAlgebra.comap (· ∈ s) ⊤ := by
...
lemma generateFrom_singleton_le {m : SigmaAlgebra α} {s : Set α} (hs : MeasurableSet s) :
    SigmaAlgebra.generateFrom {s} ≤ m :=
  generateFrom_le (fun _ ht ↦ mem_singleton_iff.1 ht ▸ hs)

lemma comap_indicator_const_le_generateFrom_singleton {M : Type*} [Zero M] [SigmaAlgebra M]
  (s : Set α) (c : M) :
    SigmaAlgebra.comap (s.indicator (fun _ ↦ c)) inferInstance ≤
      SigmaAlgebra.generateFrom {s} :=
  (measurable_const.indicator (mem_generateFrom (by simp))).comap_le
...
theorem mem_generateFrom_singleton_iff {s t : Set α} :
  ...
  have top_mem (x : Set Prop) : x ∈ (⊤ : SigmaAlgebra Prop) := by
    exact SigmaAlgebra.mem_top
  simp_rw [top_mem, true_and]
  constructor
  · rintro ⟨x, rfl⟩
    by_cases hT : True ∈ x
    · by_cases hF : False ∈ x
...
is measurable. -/
@[measurability]
lemma measurableSet_tendsto {_ : SigmaAlgebra β} [SigmaAlgebra γ]
