...
/-!
# Integration with respect to iterated product measures

This file proves Fubini's theorem for `Measure.productBySections`. This choice retains the
s-finite theory. For a unique product, `Measure.prod_eq_productBySections` transports the results.
...
* `MeasureTheory.integral_prod`: Fubini's theorem. It states that for an integrable function
  ...
  `∫ z, f z ∂(μ.productBySections ν) = ∫ x, ∫ y, f (x, y) ∂ν ∂μ`. This theorem has the same
  variants as
  Tonelli's theorem (see `MeasureTheory.lintegral_productBySections`). The lemma
...
theorem integrable_measure_prodMk_left {s : Set (α × β)} (hs : MeasurableSet s)
    (h2s : (μ.productBySections ν) s ≠ ∞) : Integrable (fun x => ν.real (Prod.mk x ⁻¹' s)) μ := by
...
protected theorem MeasureTheory.AEStronglyMeasurable.prod_swap [SFinite μ] [SFinite ν]
    {f : β × α → X} (hf : AEStronglyMeasurable f (ν.productBySections μ)) :
    AEStronglyMeasurable (fun z : α × β => f z.swap) (μ.productBySections ν) := by
  rw [← productBySections_swap] at hf
  exact hf.comp_measurable measurable_swap

theorem MeasureTheory.AEStronglyMeasurable.comp_fst [SFinite ν] {γ} [TopologicalSpace γ] {f : α → γ}
    (hf : AEStronglyMeasurable f μ) :
    AEStronglyMeasurable (fun z : α × β => f z.1) (μ.productBySections ν) :=
...
theorem MeasureTheory.AEStronglyMeasurable.comp_snd [SFinite ν] {γ} [TopologicalSpace γ] {f : β → γ}
    ...
    AEStronglyMeasurable (fun z : α × β => f z.2) (μ.productBySections ν) :=
...
theorem MeasureTheory.AEStronglyMeasurable.integral_prod_right' [SFinite ν] [NormedSpace ℝ E]
    ⦃f : α × β → E⦄ (hf : AEStronglyMeasurable f (μ.productBySections ν)) :
...
theorem MeasureTheory.AEStronglyMeasurable.prodMk_left [SFinite ν] {f : α × β → X}
    (hf : AEStronglyMeasurable f (μ.productBySections ν)) :
...
protected theorem MeasureTheory.AEStronglyMeasurable.of_comp_snd {f : β → X} [SFinite ν]
    (hf : AEStronglyMeasurable (f ·.2) (μ.productBySections ν)) (hμ : μ ≠ 0) :
    AEStronglyMeasurable f ν := by
  have := NeZero.mk hμ
...
theorem integrable_swap_iff [SFinite μ] {f : α × β → E} :
    Integrable (f ∘ Prod.swap) (ν.productBySections μ) ↔ Integrable f (μ.productBySections ν) :=
  measurePreserving_swap_productBySections.integrable_comp_emb
    MeasurableEquiv.prodComm.measurableEmbedding

theorem Integrable.swap [SFinite μ] ⦃f : α × β → E⦄ (hf : Integrable f (μ.productBySections ν)) :
    Integrable (f ∘ Prod.swap) (ν.productBySections μ) :=
  integrable_swap_iff.2 hf

theorem hasFiniteIntegral_prod_iff ⦃f : α × β → E⦄ (h1f : StronglyMeasurable f) :
    HasFiniteIntegral f (μ.productBySections ν) ↔
      (∀ᵐ x ∂μ, HasFiniteIntegral (fun y => f (x, y)) ν) ∧
        HasFiniteIntegral (fun x => ∫ y, ‖f (x, y)‖ ∂ν) μ := by
  simp only [hasFiniteIntegral_iff_enorm, lintegral_productBySections _ h1f.enorm.aemeasurable]
...
theorem hasFiniteIntegral_prod_iff' ⦃f : α × β → E⦄
    (h1f : AEStronglyMeasurable f (μ.productBySections ν)) :
    HasFiniteIntegral f (μ.productBySections ν) ↔
      (∀ᵐ x ∂μ, HasFiniteIntegral (fun y => f (x, y)) ν) ∧
        HasFiniteIntegral (fun x => ∫ y, ‖f (x, y)‖ ∂ν) μ := by
...
theorem integrable_prod_iff ⦃f : α × β → E⦄ (h1f : AEStronglyMeasurable f (μ.productBySections ν)) :
    Integrable f (μ.productBySections ν) ↔
...
theorem Integrable.prod_left_ae [SFinite μ] ⦃f : α × β → E⦄
    (hf : Integrable f (μ.productBySections ν)) :
    ∀ᵐ y ∂ν, Integrable (fun x => f (x, y)) μ :=
  ((integrable_prod_iff' hf.aestronglyMeasurable).mp hf).1

theorem Integrable.prod_right_ae [SFinite μ] ⦃f : α × β → E⦄
    (hf : Integrable f (μ.productBySections ν)) :
    ∀ᵐ x ∂μ, Integrable (fun y => f (x, y)) ν :=
  hf.swap.prod_left_ae

theorem Integrable.integral_norm_prod_left ⦃f : α × β → E⦄
    (hf : Integrable f (μ.productBySections ν)) :
...
theorem Integrable.integral_norm_prod_right [SFinite μ] ⦃f : α × β → E⦄
    (hf : Integrable f (μ.productBySections ν)) : Integrable (fun y => ∫ x, ‖f (x, y)‖ ∂μ) ν :=
...
theorem Integrable.op_fst_snd {F G : Type*} [NormedAddCommGroup F] [NormedAddCommGroup G]
    ...
    Integrable (fun z ↦ op (f z.1) (g z.2)) (μ.productBySections ν) := by
  use hop.comp_aestronglyMeasurable₂ hf.1.comp_fst hg.1.comp_snd
  rcases hop_norm with ⟨C, hC⟩
  calc
    ∫⁻ z, ‖op (f z.1) (g z.2)‖ₑ ∂μ.productBySections ν ≤
      ∫⁻ z, .ofReal C * ‖f z.1‖ₑ * ‖g z.2‖ₑ ∂μ.productBySections ν := by
      gcongr with z
    ...
    _ ≤ ∫⁻ x, ∫⁻ y, .ofReal C * ‖f x‖ₑ * ‖g y‖ₑ ∂ν ∂μ := lintegral_productBySections_le _
...
lemma Integrable.comp_fst {f : α → E} (hf : Integrable f μ) (ν : Measure β) [IsFiniteMeasure ν] :
    Integrable (fun x ↦ f x.1) (μ.productBySections ν) := by
  rw [← memLp_one_iff_integrable] at hf ⊢
  exact hf.comp_fst ν

lemma Integrable.comp_snd {f : β → E} (hf : Integrable f ν) (μ : Measure α) [IsFiniteMeasure μ] :
    Integrable (fun x ↦ f x.2) (μ.productBySections ν) := by
  rw [← memLp_one_iff_integrable] at hf ⊢
  exact hf.comp_snd μ
...
theorem Integrable.smul_prod {R : Type*} [NormedRing R] [Module R E] [IsBoundedSMul R E]
    ...
    Integrable (fun z : α × β => f z.1 • g z.2) (μ.productBySections ν) :=
...
theorem Integrable.mul_prod {L : Type*} [NormedRing L] {f : α → L} {g : β → L} (hf : Integrable f μ)
    (hg : Integrable g ν) : Integrable (fun z : α × β => f z.1 * g z.2) (μ.productBySections ν) :=
  hf.smul_prod hg

theorem IntegrableOn.swap [SFinite μ] {f : α × β → E} {s : Set α} {t : Set β}
    (hf : IntegrableOn f (s ×ˢ t) (μ.productBySections ν)) :
    IntegrableOn (f ∘ Prod.swap) (t ×ˢ s) (ν.productBySections μ) := by
  rw [IntegrableOn, ← Measure.productBySections_restrict] at hf ⊢
  exact hf.swap

theorem Integrable.of_comp_snd {f : β → E}
    (hf : Integrable (f ·.2) (μ.productBySections ν)) (hμ : μ ≠ 0) :
    Integrable f ν := by
  rcases hf with ⟨hf_meas, hf_fin⟩
  use hf_meas.of_comp_snd hμ
  have := hf_meas.enorm
  aesop (add simp [HasFiniteIntegral, lintegral_productBySections, ENNReal.mul_lt_top_iff])

theorem Integrable.of_comp_fst [SFinite μ] {f : α → E}
    (hf : Integrable (f ·.1) (μ.productBySections ν))
    (hν : ν ≠ 0) : Integrable f μ :=
  hf.swap.of_comp_snd hν

theorem Integrable.comp_snd_iff [IsFiniteMeasure μ] {f : β → E} (hμ : μ ≠ 0) :
    Integrable (f ·.2) (μ.productBySections ν) ↔ Integrable f ν :=
  ⟨(.of_comp_snd · hμ), (.comp_snd · μ)⟩

theorem Integrable.comp_fst_iff [SFinite μ] [IsFiniteMeasure ν] {f : α → E} (hν : ν ≠ 0) :
    Integrable (f ·.1) (μ.productBySections ν) ↔ Integrable f μ :=
...
theorem Integrable.integral_prod_left ⦃f : α × β → E⦄ (hf : Integrable f (μ.productBySections ν)) :
...
theorem integral_prod_swap (f : α × β → E) :
  ...
  measurePreserving_swap_productBySections.integral_comp
    MeasurableEquiv.prodComm.measurableEmbedding _

theorem setIntegral_prod_swap (s : Set α) (t : Set β) (f : α × β → E) :
    ∫ (z : β × α) in t ×ˢ s, f z.swap ∂ν.productBySections μ =
      ∫ (z : α × β) in s ×ˢ t, f z ∂μ.productBySections ν := by
  rw [← Measure.productBySections_restrict, ← Measure.productBySections_restrict,
    integral_prod_swap]
...
theorem integral_fn_integral_add ⦃f g : α × β → E⦄ (F : E → E')
    (hf : Integrable f (μ.productBySections ν))
    (hg : Integrable g (μ.productBySections ν)) :
...
theorem integral_fn_integral_sub ⦃f g : α × β → E⦄ (F : E → E')
    (hf : Integrable f (μ.productBySections ν))
    (hg : Integrable g (μ.productBySections ν)) :
    (∫ x, F (∫ y, f (x, y) - g (x, y) ∂ν) ∂μ) =
      ∫ x, F ((∫ y, f (x, y) ∂ν) - ∫ y, g (x, y) ∂ν) ∂μ := by
...
theorem lintegral_fn_integral_sub ⦃f g : α × β → E⦄ (F : E → ℝ≥0∞)
    (hf : Integrable f (μ.productBySections ν))
    (hg : Integrable g (μ.productBySections ν)) :
    (∫⁻ x, F (∫ y, f (x, y) - g (x, y) ∂ν) ∂μ) =
      ∫⁻ x, F ((∫ y, f (x, y) ∂ν) - ∫ y, g (x, y) ∂ν) ∂μ := by
...
theorem integral_integral_add ⦃f g : α × β → E⦄ (hf : Integrable f (μ.productBySections ν))
    (hg : Integrable g (μ.productBySections ν)) :
...
theorem integral_integral_sub ⦃f g : α × β → E⦄ (hf : Integrable f (μ.productBySections ν))
    (hg : Integrable g (μ.productBySections ν)) :
    (∫ x, ∫ y, f (x, y) - g (x, y) ∂ν ∂μ) = (∫ x, ∫ y, f (x, y) ∂ν ∂μ) - ∫ x, ∫ y, g (x, y) ∂ν ∂μ :=
...
theorem continuous_integral_integral :
    Continuous fun f : α × β →₁[μ.productBySections ν] E => ∫ x, ∫ y, f (x, y) ∂ν ∂μ := by
  ...
  have (i : α × β →₁[μ.productBySections ν] E) : Measurable fun z => ‖i z - g z‖ₑ :=
    ((Lp.stronglyMeasurable i).sub (Lp.stronglyMeasurable g)).enorm
  simp_rw [← lintegral_productBySections _ (this _).aemeasurable, ← L1.ofReal_norm_sub_eq_lintegral,
    ← ofReal_zero]
...
theorem integral_prod (f : α × β → E) (hf : Integrable f (μ.productBySections ν)) :
  ...
  · intro c s hs h2s
    ...
    rw [Measure.productBySections_apply hs]
...
theorem integral_prod_symm (f : α × β → E) (hf : Integrable f (μ.productBySections ν)) :
    ∫ z, f z ∂μ.productBySections ν = ∫ y, ∫ x, f (x, y) ∂μ ∂ν := by
  rw [← integral_prod_swap f]; exact integral_prod _ hf.swap

/-- Reversed version of **Fubini's Theorem**. -/
theorem integral_integral {f : α → β → E} (hf : Integrable (uncurry f) (μ.productBySections ν)) :
    ∫ x, ∫ y, f x y ∂ν ∂μ = ∫ z, f z.1 z.2 ∂μ.productBySections ν :=
  (integral_prod _ hf).symm

/-- Reversed version of **Fubini's Theorem** (symmetric version). -/
theorem integral_integral_symm {f : α → β → E}
    (hf : Integrable (uncurry f) (μ.productBySections ν)) :
...
lemma intervalIntegral_integral_swap {a b : ℝ} {f : ℝ → α → E}
    (h_int : Integrable (uncurry f) ((volume.restrict (Set.uIoc a b)).productBySections μ)) :
...
lemma intervalIntegral_intervalIntegral_swap {F : ℝ → ℝ → E} {a b c d : ℝ}
  ...
  · rwa [← integrable_swap_iff, Measure.productBySections_restrict,
      ← Measure.volume_eq_productBySections,
      ← IntegrableOn]
...
theorem setIntegral_prod (f : α × β → E) {s : Set α} {t : Set β}
  ...
  simp only [← Measure.productBySections_restrict s t, IntegrableOn] at hf ⊢
...
theorem integral_prod_bilin {E F G 𝕜 : Type*} [RCLike 𝕜]
    ...
    ∫ z, B (f z.1) (g z.2) ∂μ.productBySections ν = B (∫ x, f x ∂μ) (∫ y, g y ∂ν) := by
  have : Integrable (fun z ↦ B (f z.1) (g z.2)) (μ.productBySections ν) :=
...
theorem integral_prod_smul {𝕜 : Type*} [RCLike 𝕜] [NormedSpace 𝕜 E] (f : α → 𝕜) (g : β → E) :
  ...
  by_cases h : Integrable (fun z : α × β => f z.1 • g z.2) (μ.productBySections ν)
...
theorem integral_prod_mul {L : Type*} [RCLike L] (f : α → L) (g : β → L) :
    ∫ z, f z.1 * g z.2 ∂μ.productBySections ν = (∫ x, f x ∂μ) * ∫ y, g y ∂ν :=
  integral_prod_smul f g

theorem setIntegral_prod_mul {L : Type*} [RCLike L] (f : α → L) (g : β → L) (s : Set α)
    (t : Set β) :
    ∫ z in s ×ˢ t, f z.1 * g z.2 ∂μ.productBySections ν =
      (∫ x in s, f x ∂μ) * ∫ y in t, g y ∂ν := by
  rw [← Measure.productBySections_restrict s t]
  apply integral_prod_mul

theorem integral_fun_snd (f : β → E) :
    ∫ z, f z.2 ∂μ.productBySections ν = μ.real univ • ∫ y, f y ∂ν := by
  simpa using integral_prod_smul (1 : α → ℝ) f

theorem integral_fun_fst (f : α → E) :
    ∫ z, f z.1 ∂μ.productBySections ν = ν.real univ • ∫ x, f x ∂μ := by
...
lemma integrable_continuousLinearMap_prod'
  ...
  change Integrable (fun v ↦ L v) (μ.productBySections ν)
