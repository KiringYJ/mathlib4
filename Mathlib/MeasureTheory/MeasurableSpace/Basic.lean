...
theorem iUnion_of_strict_mono_measurable_space_is_not_a_measurable_space
  ...
  rcases exists_disjoint_measurable_set_of_strict_mono h_mono with
    ⟨F, hF_succ_measurable, hF_not_measurable, hF_disjoint⟩
  have hF_measurable_in_m' : ∀ n, MeasurableSet[m'] (F n) := by
    intro n
    have : (F n) ∈ {s | MeasurableSet[m (n + 1)] s} := by
      simpa using hF_succ_measurable n
    have : (F n) ∈ ⋃ k, {s | MeasurableSet[m k] s} :=
      mem_iUnion.mpr ⟨n + 1, this⟩
    simpa [← h_eq] using this
  let uF : Set α := ⋃ n : ℕ, F n
  have h_unionC_measurable_in_m' : MeasurableSet[m'] uF := by
    have : ∀ n, MeasurableSet[m'] (F n) := hF_measurable_in_m'
    exact MeasurableSet.iUnion this
  have h_unionC_in_chain :
      uF ∈ ⋃ k, {s | MeasurableSet[m k] s} := by
    simpa [← h_eq] using h_unionC_measurable_in_m'
  rcases mem_iUnion.mp h_unionC_in_chain with ⟨k, hk⟩
  let unionsF : Set (Set α) := { U | ∃ (S : Set ℕ), U = ⋃ n ∈ S, F n }
  let B : { n // k ≤ n } → Set α := λ ⟨n, hn⟩ => sInf { A : Set α |
    F n ⊆ A
    ∧ MeasurableSet[m n] A
    ∧ A ∈ unionsF}
  have hB_adm :
  ∀ (n : { n // k ≤ n }),
    B n ∈ { A : Set α | F n ⊆ A ∧ MeasurableSet[m n] A ∧ A ∈ unionsF } := by
    sorry
  have h_Ci_not_subset_Bj :
    ∀ {i j : ℕ}, i < j → ¬ (F i ⊆ B j) := by
    intro i j hij hsub
    -- Work with `A := B j \ C i`
    let A : Set α := B j \ F i
    ...
    have hCj_sub_Bj : F j ⊆ B j           := hB_adm_j.1
    have hBj_meas_mj : MeasurableSet[m j] (B j) := hB_adm_j.2.1
    have hBj_meas_F  : MeasurableSet[uF] (Subtype.val ⁻¹' (B j)) := hB_adm_j.2.2
