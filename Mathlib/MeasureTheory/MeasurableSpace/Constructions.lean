...
/-- **Generator lemma for atoms**.
Let `m` be a measurable space generated by an *algebra of sets* `C`.
Then the measurable atom of a point `x` (with respect to `m`) is already
