...
* `class BorelSpace` : a space with `TopologicalSpace` and `SigmaAlgebra` structures
  such that `‹SigmaAlgebra α› = borel α`;
* `class OpensSigmaAlgebra` : a space with `TopologicalSpace` and `SigmaAlgebra`
  structures such that all open sets are measurable; equivalently, `borel α ≤ ‹SigmaAlgebra α›`.
* `BorelSpace` instances on `Empty`, `Unit`, `Bool`, `Nat`, `Int`, `Rat`;
* `SigmaAlgebra` and `BorelSpace` instances on `ℝ`, `ℝ≥0`, `ℝ≥0∞`.
...
/-- `SigmaAlgebra` structure generated by `TopologicalSpace`. -/
@[instance_reducible]
def borel (α : Type u) [TopologicalSpace α] : SigmaAlgebra α :=
  generateFrom { s : Set α | IsOpen s }

theorem borel_anti : Antitone (@borel α) := fun _ _ h =>
  SigmaAlgebra.generateFrom_le fun _ hs => SigmaAlgebra.mem_generateFrom (h _ hs)

theorem borel_eq_top_of_discrete [TopologicalSpace α] [DiscreteTopology α] : borel α = ⊤ :=
  top_le_iff.1 fun _ _ => SigmaAlgebra.mem_generateFrom (isOpen_discrete _)
...
theorem borel_eq_generateFrom_of_subbasis {s : Set (Set α)} [t : TopologicalSpace α]
  ...
  le_antisymm
    (generateFrom_le fun u (hu : t.IsOpen u) => by
      rw [hs] at hu
      induction hu with
      | basic _ hu => exact SigmaAlgebra.mem_generateFrom hu
    ...
    (generateFrom_le fun u hu => SigmaAlgebra.mem_generateFrom <|
      show t.IsOpen u by rw [hs]; exact GenerateOpen.basic _ hu)
...
theorem borel_eq_generateFrom_isClosed [TopologicalSpace α] :
  ...
  le_antisymm
    (generateFrom_le fun _t ht => by
      rw [← compl_compl _t]
      exact (SigmaAlgebra.generateFrom { s : Set α | IsClosed s }).compl_mem
        (SigmaAlgebra.mem_generateFrom <| isClosed_compl_iff.2 ht))
    (generateFrom_le fun _t ht => by
      rw [← compl_compl _t]
      exact (borel α).compl_mem (SigmaAlgebra.mem_generateFrom <| isOpen_compl_iff.2 ht))
...
theorem Continuous.borel_measurable [TopologicalSpace α] [TopologicalSpace β] {f : α → β}
  ...
  Measurable.of_le_map <|
    generateFrom_le fun _ hs => SigmaAlgebra.mem_generateFrom (hs.preimage hf)

/-- A space with `SigmaAlgebra` and `TopologicalSpace` structures such that
all open sets are measurable. -/
class OpensSigmaAlgebra (α : Type*) [TopologicalSpace α] [h : SigmaAlgebra α] : Prop where
  /-- Borel-measurable sets are measurable. -/
  borel_le : borel α ≤ h

/-- A space with `SigmaAlgebra` and `TopologicalSpace` structures such that
the `σ`-algebra of measurable sets is exactly the `σ`-algebra generated by open sets. -/
class BorelSpace (α : Type*) [TopologicalSpace α] [SigmaAlgebra α] : Prop where
  /-- The measurable sets are exactly the Borel-measurable sets. -/
  sigmaAlgebra_eq : ‹SigmaAlgebra α› = borel α
...
- if `α` is a topological space with instances `[SigmaAlgebra α] [BorelSpace α]`, then
  `borelize α` replaces the former instance by `borel α`;
- otherwise, `borelize α` adds instances `borel α : SigmaAlgebra α` and `⟨rfl⟩ : BorelSpace α`.

Finally, `borelize α β γ` runs `borelize α; borelize β; borelize γ`.
-/
syntax "borelize" (ppSpace colGt term:max)* : tactic

/-- Add instances `borel e : SigmaAlgebra e` and `⟨rfl⟩ : BorelSpace e`. -/
meta def addBorelInstance (e : Expr) : TacticM Unit := do
  let t ← Lean.Elab.Term.exprToSyntax e
  evalTactic <| ← `(tactic|
    refine_lift
      letI : SigmaAlgebra $t := borel $t
      haveI : BorelSpace $t := ⟨rfl⟩
      ?_)

/-- Given a type `e`, an assumption `i : SigmaAlgebra e`, and an instance `[BorelSpace e]`,
replace `i` with `borel e`. -/
meta def borelToRefl (e : Expr) (i : FVarId) : TacticM Unit := do
  let te ← Lean.Elab.Term.exprToSyntax e
  evalTactic <| ← `(tactic|
    have := @BorelSpace.sigmaAlgebra_eq $te _ _ _)
  try
    ...
    throwError m!"\
      ...
      depends on\n\
      {Expr.fvar i} : SigmaAlgebra {e}`\n\
      so `borelize` isn't available"
  evalTactic <| ← `(tactic|
    refine_lift
      letI : SigmaAlgebra $te := borel $te
      ?_)

/-- Given a type `$t`, if there is an assumption `[i : SigmaAlgebra $t]`, then try to prove
`[BorelSpace $t]` and replace `i` with `borel $t`. Otherwise, add instances
`borel $t : SigmaAlgebra $t` and `⟨rfl⟩ : BorelSpace $t`. -/
meta def borelize (t : Term) : TacticM Unit := withMainContext do
  ...
  let i? ← findLocalDeclWithType? (← mkAppOptM ``SigmaAlgebra #[e])
...
instance (priority := 100) OrderDual.opensSigmaAlgebra {α : Type*} [TopologicalSpace α]
    [SigmaAlgebra α] [h : OpensSigmaAlgebra α] : OpensSigmaAlgebra αᵒᵈ where
  borel_le := h.borel_le

instance (priority := 100) OrderDual.borelSpace {α : Type*} [TopologicalSpace α]
    [SigmaAlgebra α] [h : BorelSpace α] : BorelSpace αᵒᵈ where
  sigmaAlgebra_eq := h.sigmaAlgebra_eq

/-- In a `BorelSpace` all open sets are measurable. -/
instance (priority := 100) BorelSpace.opensSigmaAlgebra {α : Type*} [TopologicalSpace α]
    [SigmaAlgebra α] [BorelSpace α] : OpensSigmaAlgebra α :=
  ⟨ge_of_eq <| BorelSpace.sigmaAlgebra_eq⟩

instance Subtype.borelSpace {α : Type*} [TopologicalSpace α] [SigmaAlgebra α]
    [hα : BorelSpace α] (p : α → Prop) : BorelSpace (Subtype p) :=
  ⟨by borelize α; symm; apply borel_comap⟩

instance Countable.instBorelSpace [Countable α] [SigmaAlgebra α] [MeasurableSingletonClass α]
  ...
  have : ∀ s, @MeasurableSet α (borel α) s := fun s ↦
    SigmaAlgebra.mem_generateFrom (isOpen_discrete s)
  exact ⟨by aesop⟩

instance Subtype.opensSigmaAlgebra {α : Type*} [TopologicalSpace α] [SigmaAlgebra α]
    [h : OpensSigmaAlgebra α] (p : α → Prop) : OpensSigmaAlgebra (Subtype p) :=
  ⟨by
    rw [borel_comap]
    exact comap_mono h.1⟩

lemma opensSigmaAlgebra_iff_forall_measurableSet
    [TopologicalSpace α] [SigmaAlgebra α] :
    OpensSigmaAlgebra α ↔ (∀ (s : Set α), IsOpen s → MeasurableSet s) := by
  refine ⟨fun h s hs ↦ ?_, fun h ↦ ⟨generateFrom_le h⟩⟩
  exact OpensSigmaAlgebra.borel_le (SigmaAlgebra.mem_generateFrom hs)

instance (priority := 100) BorelSpace.countablyGenerated {α : Type*} [TopologicalSpace α]
    [SigmaAlgebra α] [BorelSpace α] [SecondCountableTopology α] : CountablyGenerated α := by
...
variable [TopologicalSpace α] [SigmaAlgebra α] [OpensSigmaAlgebra α] [TopologicalSpace β]
  [SigmaAlgebra β] [OpensSigmaAlgebra β] [TopologicalSpace γ] [SigmaAlgebra γ]
  [BorelSpace γ] [TopologicalSpace γ₂] [SigmaAlgebra γ₂] [BorelSpace γ₂] [SigmaAlgebra δ]

theorem IsOpen.measurableSet (h : IsOpen s) : MeasurableSet s :=
  OpensSigmaAlgebra.borel_le (SigmaAlgebra.mem_generateFrom h)
...
theorem MeasurableSet.induction_on_open {C : ∀ s : Set γ, MeasurableSet s → Prop}
  ...
  SigmaAlgebra.induction_on_inter BorelSpace.sigmaAlgebra_eq isPiSystem_isOpen
...
theorem measurable_of_isOpen {f : δ → γ} (hf : ∀ s, IsOpen s → MeasurableSet (f ⁻¹' s)) :
    Measurable f := by
  rw [‹BorelSpace γ›.sigmaAlgebra_eq]
...
instance (priority := 100) OpensSigmaAlgebra.separatesPoints [T0Space α] :
...
instance (priority := 100) OpensSigmaAlgebra.toMeasurableSingletonClass [T1Space α] :
    MeasurableSingletonClass α :=
  ⟨fun _ => isClosed_singleton.measurableSet⟩

instance Pi.opensSigmaAlgebra {ι : Type*} {X : ι → Type*} [Countable ι]
    [t' : ∀ i, TopologicalSpace (X i)] [∀ i, SigmaAlgebra (X i)]
    [∀ i, SecondCountableTopology (X i)] [∀ i, OpensSigmaAlgebra (X i)] :
    OpensSigmaAlgebra (∀ i, X i) := by
  constructor
...
/-- This is not covered by `Pi.opensSigmaAlgebra` as it does not require second countability. -/
instance Pi.opensSigmaAlgebra_of_subsingleton {ι : Type*} {X : ι → Type*} [Subsingleton ι]
    [∀ i, TopologicalSpace (X i)] [∀ i, SigmaAlgebra (X i)] [∀ i, OpensSigmaAlgebra (X i)] :
    OpensSigmaAlgebra (∀ i, X i) where
  borel_le := by
    obtain h | h := isEmpty_or_nonempty ι
    · exact fun s _ ↦ Subsingleton.set_cases
        (SigmaAlgebra.empty_mem _) (SigmaAlgebra.univ_mem _) s
    have := Classical.choice (nonempty_unique ι)
    rw [borel, SigmaAlgebra.pi, ciSup_unique]
    refine SigmaAlgebra.generateFrom_le fun s hs ↦ SigmaAlgebra.mem_comap.2 ?_
...
instance Prod.opensSigmaAlgebra [h : SecondCountableTopologyEither α β] :
    OpensSigmaAlgebra (α × β) := by
  apply opensSigmaAlgebra_iff_forall_measurableSet.2 (fun s hs ↦ ?_)
...
instance separatesPointsOfOpensSigmaAlgebraOfT0Space [T0Space α] :
    SigmaAlgebra.SeparatesPoints α where
  separates x y := by
    contrapose!
    exact fun a => exists_measurableSet_of_ne a

/-- A continuous function from an `OpensSigmaAlgebra` to a `BorelSpace`
is measurable. -/
@[fun_prop]
theorem Continuous.measurable {f : α → γ} (hf : Continuous f) : Measurable f :=
  hf.borel_measurable.mono OpensSigmaAlgebra.borel_le (le_of_eq <| BorelSpace.sigmaAlgebra_eq)
...
instance (priority := 100) ContinuousConstSMul.toMeasurableConstSMul {M α} [TopologicalSpace α]
    [SigmaAlgebra α] [BorelSpace α] [SMul M α] [ContinuousConstSMul M α] :
    MeasurableConstSMul M α where

@[to_additive]
instance (priority := 100) ContinuousSMul.toMeasurableSMul {M α} [TopologicalSpace M]
    [TopologicalSpace α] [SigmaAlgebra M] [SigmaAlgebra α] [OpensSigmaAlgebra M]
...
instance (priority := 100) ContinuousSMul.measurableSMul₂ {M α} [TopologicalSpace M]
    [SigmaAlgebra M] [OpensSigmaAlgebra M] [TopologicalSpace α]
    [SecondCountableTopologyEither M α] [SigmaAlgebra α] [BorelSpace α] [SMul M α]
...
variable [TopologicalSpace α] [mα : SigmaAlgebra α] [BorelSpace α] [mβ : TopologicalSpace β]
  [SigmaAlgebra β] [BorelSpace β] [TopologicalSpace γ] [SigmaAlgebra γ] [BorelSpace γ]
  [SigmaAlgebra δ]

theorem pi_le_borel_pi {ι : Type*} {X : ι → Type*} [∀ i, TopologicalSpace (X i)]
    [∀ i, SigmaAlgebra (X i)] [∀ i, BorelSpace (X i)] :
      SigmaAlgebra.pi ≤ borel (∀ i, X i) := by
  have : ‹∀ i, SigmaAlgebra (X i)› = fun i => borel (X i) :=
    funext fun i => BorelSpace.sigmaAlgebra_eq
  rw [this]
  exact iSup_le fun i => comap_le_iff_le_map.2 <| (continuous_apply i).borel_measurable

theorem prod_le_borel_prod : Prod.instSigmaAlgebra ≤ borel (α × β) := by
  rw [‹BorelSpace α›.sigmaAlgebra_eq, ‹BorelSpace β›.sigmaAlgebra_eq]
  refine sup_le ?_ ?_
  · exact comap_le_iff_le_map.mpr continuous_fst.borel_measurable
  · exact comap_le_iff_le_map.mpr continuous_snd.borel_measurable

instance Pi.borelSpace {ι : Type*} {X : ι → Type*} [Countable ι] [∀ i, TopologicalSpace (X i)]
    [∀ i, SigmaAlgebra (X i)] [∀ i, SecondCountableTopology (X i)] [∀ i, BorelSpace (X i)] :
    BorelSpace (∀ i, X i) :=
  ⟨le_antisymm pi_le_borel_pi OpensSigmaAlgebra.borel_le⟩
...
instance Pi.borelSpace_of_subsingleton {ι : Type*} {X : ι → Type*} [Subsingleton ι]
    [∀ i, TopologicalSpace (X i)] [∀ i, SigmaAlgebra (X i)] [∀ i, BorelSpace (X i)] :
    BorelSpace (∀ i, X i) :=
  ⟨le_antisymm pi_le_borel_pi OpensSigmaAlgebra.borel_le⟩

instance Prod.borelSpace [SecondCountableTopologyEither α β] :
    BorelSpace (α × β) :=
  ⟨le_antisymm prod_le_borel_prod OpensSigmaAlgebra.borel_le⟩
...
lemma MeasurableEmbedding.borelSpace {α β : Type*} [SigmaAlgebra α] [TopologicalSpace α]
    [SigmaAlgebra β] [TopologicalSpace β] [hβ : BorelSpace β] {e : α → β}
    (h'e : MeasurableEmbedding e) (h''e : IsInducing e) :
    BorelSpace α := by
  constructor
  have : SigmaAlgebra.comap e (borel β) = ‹_› := by
    simpa [hβ.sigmaAlgebra_eq] using h'e.comap_eq
...
instance DiscreteSigmaAlgebra.toBorelSpace {α : Type*} [TopologicalSpace α] [DiscreteTopology α]
    [SigmaAlgebra α] [DiscreteSigmaAlgebra α] : BorelSpace α := by
  constructor
  rw [borel_eq_top_of_discrete]
  exact top_unique fun _ _ ↦ MeasurableSet.of_discrete
