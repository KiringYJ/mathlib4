...
/-- The sets in a σ-algebra form a π-system. -/
theorem isPiSystem {α : Type*} (m : SigmaAlgebra α) : IsPiSystem (m : Set (Set α)) :=
  fun _ hs _ ht _ => m.inter_mem hs ht
...
theorem generatePiSystem_subset {m : SigmaAlgebra α} {S : Set (Set α)}
    (hS : S ⊆ m) : generatePiSystem S ⊆ m := by
  intro t h_in_pi
  induction h_in_pi using generatePiSystem_induction with
  | base _ hs => exact hS hs
  | inter _ _ _ _ _ hs ht => exact m.inter_mem hs ht

theorem mem_generateFrom_of_mem_generatePiSystem {g : Set (Set α)} {t : Set α}
    (ht : t ∈ generatePiSystem g) : t ∈ generateFrom g :=
  generatePiSystem_subset (fun _ hs => mem_generateFrom hs) ht
...
theorem generateFrom_generatePiSystem_eq {g : Set (Set α)} :
  ...
  · exact fun _ ht => mem_generateFrom_of_mem_generatePiSystem ht
  · exact fun t h_t => mem_generateFrom (subset_generatePiSystem_self _ h_t)
...
theorem generateFrom_piiUnionInter_singleton_left (s : ι → Set α) (S : Set ι) :
  ...
  · rintro _ ⟨I, hI, f, hf, rfl⟩
    refine Finset.measurableSet_biInter _ fun m hm => mem_generateFrom ?_
...
theorem generateFrom_piiUnionInter_le {m : SigmaAlgebra α} (π : ι → Set (Set α))
    (h : ∀ n, generateFrom (π n) ≤ m) (S : Set ι) : generateFrom (piiUnionInter π S) ≤ m := by
  refine generateFrom_le ?_
  rintro t ⟨ht_p, _, ft, hft_mem_pi, rfl⟩
  refine Finset.measurableSet_biInter _ fun x hx_mem => (h x) ?_
  exact mem_generateFrom (hft_mem_pi x hx_mem)
...
theorem mem_piiUnionInter_of_mem (m : ι → SigmaAlgebra α) {S : Set ι} {i : ι}
    (hiS : i ∈ S) {s : Set α} (hs : s ∈ m i) :
    s ∈ piiUnionInter (fun n => (m n : Set (Set α))) S :=
...
theorem mem_iSup_of_mem_piiUnionInter (m : ι → SigmaAlgebra α) (S : Set ι) {t : Set α}
    (ht : t ∈ piiUnionInter (fun n => (m n : Set (Set α))) S) : t ∈ ⨆ i ∈ S, m i := by
  rcases ht with ⟨pt, hpt, ft, ht_m, rfl⟩
  refine pt.measurableSet_biInter fun i hi => ?_
  suffices h_le : m i ≤ ⨆ i ∈ S, m i from h_le (ht_m i hi)
  have hi' : i ∈ S := hpt hi
  exact le_iSup₂ (f := fun i (_ : i ∈ S) => m i) i hi'

theorem generateFrom_piiUnionInter (m : ι → SigmaAlgebra α) (S : Set ι) :
    generateFrom (piiUnionInter (fun n => (m n : Set (Set α))) S) = ⨆ i ∈ S, m i := by
  refine le_antisymm ?_ ?_
  · rw [← generateFrom_self (⨆ i ∈ S, m i)]
    exact generateFrom_mono fun _ ht => mem_iSup_of_mem_piiUnionInter m S ht
  · refine iSup₂_le fun i hi => ?_
    rw [← generateFrom_self (m i)]
    exact generateFrom_mono fun _ ht => mem_piiUnionInter_of_mem m hi ht
...
structure DynkinSystem (α : Type*) where
  /-- The family of sets in the Dynkin system. -/
  carrier : Set (Set α)
  ...
  `SigmaAlgebra.DynkinSystem.iUnion_mem` instead. -/
  iUnion_mem_nat : ∀ {f : ℕ → Set α}, Pairwise (Disjoint on f) →
    (∀ i, f i ∈ carrier) → ⋃ i, f i ∈ carrier

namespace DynkinSystem

/-- A Dynkin system is naturally viewed as a collection of sets. -/
instance : SetLike (DynkinSystem α) (Set α) where
  coe := DynkinSystem.carrier
...
@[ext]
theorem ext {d₁ d₂ : DynkinSystem α} (h : ∀ s : Set α, s ∈ d₁ ↔ s ∈ d₂) : d₁ = d₂ :=
  SetLike.ext fun s ↦ h s
...
theorem univ_mem : univ ∈ d := by
  change univ ∈ d.carrier
  simpa using d.compl_mem d.empty_mem

theorem iUnion_mem {β} [Countable β] {f : β → Set α} (hd : Pairwise (Disjoint on f))
    (h : ∀ i, f i ∈ d) : ⋃ i, f i ∈ d := by
  cases nonempty_encodable β
  rw [← Encodable.iUnion_decode₂]
  exact
    d.iUnion_mem_nat (Encodable.iUnion_decode₂_disjoint_on hd) fun n =>
      Encodable.iUnion_decode₂_cases d.empty_mem h

theorem union_mem {s₁ s₂ : Set α} (h₁ : s₁ ∈ d) (h₂ : s₂ ∈ d) (h : Disjoint s₁ s₂) :
    s₁ ∪ s₂ ∈ d := by
  rw [union_eq_iUnion]
  exact d.iUnion_mem (pairwise_disjoint_on_bool.2 h) (Bool.forall_bool.2 ⟨h₂, h₁⟩)

theorem sdiff_mem {s₁ s₂ : Set α} (h₁ : s₁ ∈ d) (h₂ : s₂ ∈ d) (h : s₂ ⊆ s₁) :
    s₁ \ s₂ ∈ d := by
  apply d.compl_mem_iff.1
  simp only [sdiff_eq, compl_inter, compl_compl]
  exact d.union_mem (d.compl_mem h₁) h₂ (disjoint_compl_left.mono_right h)

instance instLEDynkinSystem : LE (DynkinSystem α) where le d₁ d₂ := d₁.carrier ⊆ d₂.carrier

theorem le_def {a b : DynkinSystem α} : a ≤ b ↔ (a : Set (Set α)) ⊆ b :=
  Iff.rfl
...
instance : PartialOrder (DynkinSystem α) :=
  { DynkinSystem.instLEDynkinSystem with
    ...
    le_antisymm := fun _ _ h₁ h₂ => ext fun _ => ⟨fun hs ↦ h₁ hs, fun hs ↦ h₂ hs⟩ }

/-- Every measurable space (σ-algebra) forms a Dynkin system -/
def ofSigmaAlgebra (m : SigmaAlgebra α) : DynkinSystem α where
  carrier := m.carrier
  empty_mem := m.empty_mem
  compl_mem := m.compl_mem
  iUnion_mem_nat _ hf := m.iUnion_mem hf

theorem ofSigmaAlgebra_le_ofSigmaAlgebra_iff {m₁ m₂ : SigmaAlgebra α} :
    ofSigmaAlgebra m₁ ≤ ofSigmaAlgebra m₂ ↔ m₁ ≤ m₂ :=
  Iff.rfl
...
def generate (s : Set (Set α)) : DynkinSystem α where
  carrier := GenerateHas s
  empty_mem := GenerateHas.empty
  compl_mem := GenerateHas.compl
  iUnion_mem_nat := GenerateHas.iUnion

theorem generate_carrier {C : Set (Set α)} : (generate C : Set (Set α)) = GenerateHas C :=
  rfl
...
@[instance_reducible]
def toSigmaAlgebra (h_inter : ∀ s₁ s₂, s₁ ∈ d → s₂ ∈ d → s₁ ∩ s₂ ∈ d) :
    SigmaAlgebra α where
  carrier := d.carrier
  isSigmaAlgebra :=
    { empty_mem := d.empty_mem
      compl_mem := fun {_} hs ↦ d.compl_mem hs
      iUnion_mem_nat := fun f hf ↦ by
        rw [← iUnion_disjointed]
        exact
          d.iUnion_mem (disjoint_disjointed _) fun n =>
            disjointedRec (fun (t : Set α) i h => h_inter _ _ h <| d.compl_mem <| hf i) (hf n) }

theorem ofSigmaAlgebra_toSigmaAlgebra
    (h_inter : ∀ s₁ s₂, s₁ ∈ d → s₂ ∈ d → s₁ ∩ s₂ ∈ d) :
    ofSigmaAlgebra (d.toSigmaAlgebra h_inter) = d := by
  apply ext
  intro s
  change s ∈ d.carrier ↔ s ∈ d.carrier
  rfl

/-- If `s` is in a Dynkin system `d`, we can form the new Dynkin system `{s ∩ t | t ∈ d}`. -/
def restrictOn {s : Set α} (h : s ∈ d) : DynkinSystem α where
  carrier := {t | t ∩ s ∈ d.carrier}
  empty_mem := by simpa using d.empty_mem
  compl_mem := by
    intro t hts
    change tᶜ ∩ s ∈ d.carrier
    have : tᶜ ∩ s = (t ∩ s)ᶜ \ sᶜ := Set.ext fun x => by by_cases h : x ∈ s <;> simp [h]
    rw [this]
    exact
      d.sdiff_mem (d.compl_mem hts) (d.compl_mem (show s ∈ d.carrier from h))
        (compl_subset_compl.mpr inter_subset_right)
  iUnion_mem_nat := by
    intro f hd hf
    change (⋃ i, f i) ∩ s ∈ d.carrier
    rw [iUnion_inter]
    refine d.iUnion_mem_nat ?_ hf
    exact hd.mono fun i j => Disjoint.mono inter_subset_left inter_subset_left

theorem generate_le {s : Set (Set α)} (h : s ⊆ (d : Set (Set α))) : generate s ≤ d := by
  intro t ht
  exact ht.recOn (fun _ hs => h hs) d.empty_mem (fun {_} _ hs => d.compl_mem hs)
    fun {_} hd _ hf => d.iUnion_mem_nat hd hf

theorem generate_le_ofSigmaAlgebra_generateFrom {C : Set (Set α)} :
    generate C ≤ ofSigmaAlgebra (generateFrom C) :=
  generate_le _ fun _ hs => mem_generateFrom hs

theorem generate_inter {s : Set (Set α)} (hs : IsPiSystem s) {t₁ t₂ : Set α}
    (ht₁ : t₁ ∈ generate s) (ht₂ : t₂ ∈ generate s) : t₁ ∩ t₂ ∈ generate s :=
  have : generate s ≤ (generate s).restrictOn ht₂ :=
    generate_le _ fun s₁ hs₁ =>
      have : s₁ ∈ generate s := GenerateHas.basic s₁ hs₁
      have : generate s ≤ (generate s).restrictOn this :=
        generate_le _ fun s₂ hs₂ =>
          show s₂ ∩ s₁ ∈ generate s from
            (s₂ ∩ s₁).eq_empty_or_nonempty.elim (fun h => h.symm ▸ GenerateHas.empty) fun h =>
              GenerateHas.basic _ <| hs _ hs₂ _ hs₁ h
      have : t₂ ∩ s₁ ∈ generate s := this ht₂
      show s₁ ∩ t₂ ∈ generate s by rwa [inter_comm]
  this ht₁
...
theorem generateFrom_eq {s : Set (Set α)} (hs : IsPiSystem s) :
    generateFrom s = (generate s).toSigmaAlgebra fun _ _ => generate_inter hs :=
  le_antisymm (generateFrom_le fun t ht => by
      change t ∈ (generate s).carrier
      exact GenerateHas.basic t ht)
    (ofSigmaAlgebra_le_ofSigmaAlgebra_iff.mp <| by
      rw [ofSigmaAlgebra_toSigmaAlgebra]
      exact generate_le _ fun t ht => mem_generateFrom ht)

/-- **Dynkin's π-λ theorem** in its usual textbook form: a Dynkin system containing a
π-system contains every set in the σ-algebra generated by that π-system. -/
theorem pi_lambda {s : Set (Set α)} (hs : IsPiSystem s) (h : s ⊆ d) {t : Set α}
    (ht : t ∈ generateFrom s) : t ∈ d := by
  apply (generate_le d fun u hu => h hu)
  have ht' : t ∈ (generate s).toSigmaAlgebra fun _ _ => generate_inter hs := by
    rwa [← generateFrom_eq hs]
  exact (show t ∈ (generate s).carrier from ht')
...
@[elab_as_elim]
theorem induction_on_inter {m : SigmaAlgebra α} {C : ∀ s : Set α, s ∈ m → Prop}
    {s : Set (Set α)} (h_eq : m = generateFrom s) (h_inter : IsPiSystem s)
    (empty : C ∅ m.empty_mem)
    (basic : ∀ t (ht : t ∈ s), C t <| h_eq ▸ mem_generateFrom ht)
    (compl : ∀ t (ht : t ∈ m), C t ht → C tᶜ (m.compl_mem ht))
    (iUnion : ∀ (f : ℕ → Set α), Pairwise (Disjoint on f) → ∀ (hf : ∀ i, f i ∈ m),
      (∀ i, C (f i) (hf i)) → C (⋃ i, f i) (m.iUnion_mem hf)) :
    ∀ t (ht : t ∈ m), C t ht := by
  intro t ht
  have eq (t : Set α) : t ∈ m ↔ DynkinSystem.GenerateHas s t := by
    rw [h_eq, DynkinSystem.generateFrom_eq h_inter]
    change t ∈ ((DynkinSystem.generate s).toSigmaAlgebra fun _ _ =>
      DynkinSystem.generate_inter h_inter).carrier ↔ DynkinSystem.GenerateHas s t
    rfl
  have hs' : DynkinSystem.GenerateHas s t := (eq t).mp ht
  induction hs' with
  | basic u hu => simpa only using basic u hu
  | empty => simpa only using empty
  | @compl u hu ih => simpa only using compl u ((eq u).mpr hu) (ih ((eq u).mpr hu))
  | @iUnion f hfd hf ih =>
      simpa only using iUnion f hfd (fun n ↦ (eq (f n)).mpr (hf n)) fun n ↦ by
        simpa only using ih n ((eq (f n)).mpr (hf n))
