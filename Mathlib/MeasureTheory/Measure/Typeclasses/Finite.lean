...
open Set MeasureTheory Measure Filter Function SigmaAlgebra ENNReal

variable {α β δ ι : Type*}

namespace MeasureTheory

variable {m0 : SigmaAlgebra α} [mβ : SigmaAlgebra β] {μ ν ν₁ ν₂ : Measure α}
  {s t : Set α}
...
lemma tendsto_measure_biUnion_Ici_zero_of_pairwise_disjoint
    {X : Type*} [SigmaAlgebra X] {μ : Measure X} [IsFiniteMeasure μ]
...
def FiniteAtFilter {_m0 : SigmaAlgebra α} (μ : Measure α) (f : Filter α) : Prop :=
  ∃ s ∈ f, μ s < ∞

theorem finiteAtFilter_of_finite {_m0 : SigmaAlgebra α} (μ : Measure α) [IsFiniteMeasure μ]
...
theorem finiteAtBot {m0 : SigmaAlgebra α} (μ : Measure α) : μ.FiniteAtFilter ⊥ :=
...
structure FiniteSpanningSetsIn {m0 : SigmaAlgebra α} (μ : Measure α) (C : Set (Set α)) where
...
theorem ext_on_sigmaAlgebra_of_generate_finite {α} (m₀ : SigmaAlgebra α) {μ ν : Measure α}
    [IsFiniteMeasure μ] (C : Set (Set α)) (hμν : ∀ s ∈ C, μ s = ν s) {m : SigmaAlgebra α}
    (h : m ≤ m₀) (hA : m = SigmaAlgebra.generateFrom C) (hC : IsPiSystem C)
    (h_univ : μ Set.univ = ν Set.univ) {s : Set α} (hs : s ∈ m) : μ s = ν s := by
  ...
  | compl t htm iht =>
    rw [measure_compl (h htm) (by finiteness), measure_compl (h htm) (by finiteness), iht,
      h_univ]
  | iUnion f hfd hfm ihf =>
    rw [measure_iUnion hfd (fun i ↦ h (hfm i)), measure_iUnion hfd (fun i ↦ h (hfm i))]
...
theorem ext_of_generate_finite (C : Set (Set α)) (hA : m0 = generateFrom C) (hC : IsPiSystem C)
  ...
  Measure.ext fun _s hs =>
    ext_on_sigmaAlgebra_of_generate_finite m0 C hμν le_rfl hA hC h_univ hs
...
theorem finiteAt_nhdsWithin [TopologicalSpace α] {_m0 : SigmaAlgebra α} (μ : Measure α)
...
theorem isLocallyFiniteMeasure_of_le [TopologicalSpace α] {_m : SigmaAlgebra α} {μ ν : Measure α}
...
variable [TopologicalSpace α] [SigmaAlgebra α] {μ : Measure α} {s : Set α}
...
instance (priority := 100) isFiniteMeasureOnCompacts_of_isLocallyFiniteMeasure [TopologicalSpace α]
    {_ : SigmaAlgebra α} {μ : Measure α} [IsLocallyFiniteMeasure μ] :
...
theorem isFiniteMeasure_iff_isFiniteMeasureOnCompacts_of_compactSpace [TopologicalSpace α]
    [SigmaAlgebra α] {μ : Measure α} [CompactSpace α] :
...
noncomputable irreducible_def MeasureTheory.Measure.finiteSpanningSetsInOpen' [TopologicalSpace α]
  [SecondCountableTopology α] {m : SigmaAlgebra α} (μ : Measure α) [IsLocallyFiniteMeasure μ] :
...
variable [Preorder α] [TopologicalSpace α] [CompactIccSpace α] {m : SigmaAlgebra α}
