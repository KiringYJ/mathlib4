...
We prove that if `X` is `SigmaAlgebra.CountablyGenerated` and `μ` is s-finite, then `μ`
...
open SigmaAlgebra Set ENNReal TopologicalSpace symmDiff Real

namespace MeasureTheory

variable {X E : Type*} [m : SigmaAlgebra X] [NormedAddCommGroup E] {μ : Measure X}
...
theorem Measure.MeasureDense.of_generateFrom_isSetAlgebra_finite [IsFiniteMeasure μ]
    (h𝒜 : IsSetAlgebra 𝒜) (hgen : m = SigmaAlgebra.generateFrom 𝒜) : μ.MeasureDense 𝒜 where
  measurable s hs := hgen ▸ SigmaAlgebra.mem_generateFrom hs
  approx s ms := by
    ...
    have : MeasurableSet s ∧ ∀ (ε : ℝ), 0 < ε → ∃ t ∈ 𝒜, μ.real (s ∆ t) < ε := by
      rw [hgen] at ms
      induction ms using generateFrom_induction with
      -- If `t ∈ 𝒜`, then `μ (t ∆ t) = 0` which is less than any `ε > 0`.
      | basic t t_mem =>
        exact ⟨hgen ▸ SigmaAlgebra.mem_generateFrom t_mem, fun ε ε_pos ↦ ⟨t, t_mem, by simpa⟩⟩
...
theorem Measure.MeasureDense.of_generateFrom_isSetAlgebra_sigmaFinite (h𝒜 : IsSetAlgebra 𝒜)
    (S : μ.FiniteSpanningSetsIn 𝒜) (hgen : m = SigmaAlgebra.generateFrom 𝒜) :
        ...
        _ < ENNReal.ofReal (ε / 2) + ENNReal.ofReal (ε / 2) := by
              ...
              · rw [measure_sdiff
                    ...
                    (MeasurableSet.nullMeasurableSet
                      (MeasurableSet.inter ms
                        (hgen ▸ SigmaAlgebra.mem_generateFrom (T_mem N))))
              ...
              · rwa [← μ.restrict_apply' (hgen ▸ SigmaAlgebra.mem_generateFrom (T_mem N))]
  ...
  exists_countable_measureDense := by
    ...
    · induction hs using generateSetAlgebra_induction with
      | base t t_mem =>
        ...
        · exact hgen ▸ SigmaAlgebra.mem_generateFrom t_mem
