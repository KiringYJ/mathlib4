...
For arbitrary measures, `Measure.primitiveProd` is the maximal rectangle-cover construction.
`Measure.productBySections` is the ordered section-integral construction, defined under
`HasMeasurableSections`. Its s-finite Tonelli theory is exported from `Measure.ProductBySections`.
