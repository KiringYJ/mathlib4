...
variable {X Y Z : Type*} [mY : SigmaAlgebra Y] {f : X → Y} {g : X → Z}
...
theorem _root_.Measurable.factorsThrough [SigmaAlgebra Z] [MeasurableSingletonClass Z]
...
theorem StronglyMeasurable.exists_eq_measurable_comp [Nonempty Z] [TopologicalSpace Z]
  ...
  let mX : SigmaAlgebra X := mY.comap f
...
theorem _root_.Measurable.exists_eq_measurable_comp [Nonempty Z] [SigmaAlgebra Z]
...
variable {ι : Type*} {X : ι → Type*} [∀ i, SigmaAlgebra (X i)] {f : (Π i, X i) → Z}
...
theorem _root_.Measurable.dependsOn_of_piLE [SigmaAlgebra Z] [MeasurableSingletonClass Z]
...
/-- If a function is measurable with respect to the σ-algebra generated by the
first coordinates, then it only depends on those first coordinates. -/
theorem _root_.Measurable.dependsOn_of_piFinset [SigmaAlgebra Z] [MeasurableSingletonClass Z]
