...
* `MeasureTheory.IsSetRing.toGeneralizedBooleanSubalgebra`: the abstract generalized Boolean
  subalgebra carried by a ring of sets.
* `MeasureTheory.IsSetSemiring.supClosureSubalgebra`: the ring generated by a semiring, with its
  least-subalgebra universal property.
...
/-- Bundle a ring of sets as a generalized Boolean subalgebra of the powerset algebra. -/
@[expose] def toGeneralizedBooleanSubalgebra (hC : IsSetRing C) :
    GeneralizedBooleanSubalgebra (Set α) :=
  GeneralizedBooleanSubalgebra.ofBotSupSDiff C hC.empty_mem
    (fun _ hs _ ht ↦ hC.union_mem hs ht) hC.sdiff_mem

@[simp]
theorem coe_toGeneralizedBooleanSubalgebra (hC : IsSetRing C) :
    (hC.toGeneralizedBooleanSubalgebra : Set (Set α)) = C := rfl

end IsSetRing

/-- A generalized Boolean subalgebra of a powerset is a ring of sets. -/
theorem _root_.GeneralizedBooleanSubalgebra.isSetRing
    (C : GeneralizedBooleanSubalgebra (Set α)) : IsSetRing (C : Set (Set α)) where
  empty_mem := C.bot_mem
  union_mem := by
    intro s t hs ht
    exact C.sup_mem hs ht
  sdiff_mem := by
    intro s t hs ht
    exact C.sdiff_mem hs ht

@[simp]
theorem _root_.GeneralizedBooleanSubalgebra.toGeneralizedBooleanSubalgebra_isSetRing
    (C : GeneralizedBooleanSubalgebra (Set α)) :
    C.isSetRing.toGeneralizedBooleanSubalgebra = C := by ext; rfl
...
/-- The ring generated by a semiring of sets, bundled as a generalized Boolean subalgebra.
Its carrier is the existing finite-union closure `supClosure C`. -/
@[expose] def supClosureSubalgebra (hC : IsSetSemiring C) :
    GeneralizedBooleanSubalgebra (Set α) :=
  hC.isSetRing_supClosure.toGeneralizedBooleanSubalgebra

@[simp]
theorem coe_supClosureSubalgebra (hC : IsSetSemiring C) :
    (hC.supClosureSubalgebra : Set (Set α)) = supClosure C := rfl

/-- For a semiring of sets, finite-union closure agrees with generalized Boolean subalgebra
closure. -/
@[simp]
theorem supClosureSubalgebra_eq_closure (hC : IsSetSemiring C) :
    hC.supClosureSubalgebra = GeneralizedBooleanSubalgebra.closure C := by
  apply le_antisymm
  · exact supClosure_min GeneralizedBooleanSubalgebra.subset_closure
      (GeneralizedBooleanSubalgebra.closure C).supClosed
  · exact GeneralizedBooleanSubalgebra.closure_le.2 subset_supClosure

/-- The finite-union closure of a semiring is the least generalized Boolean subalgebra
containing it. -/
theorem supClosureSubalgebra_le_iff (hC : IsSetSemiring C)
    (R : GeneralizedBooleanSubalgebra (Set α)) : hC.supClosureSubalgebra ≤ R ↔ C ⊆ R := by
  rw [hC.supClosureSubalgebra_eq_closure, GeneralizedBooleanSubalgebra.closure_le]
