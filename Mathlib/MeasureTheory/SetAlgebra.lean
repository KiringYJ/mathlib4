...
def generateSetAlgebra {α : Type*} (𝒜 : Set (Set α)) : Set (Set α) :=
  {s | GenerateSetAlgebra 𝒜 s}
...
theorem self_subset_generateSetAlgebra : 𝒜 ⊆ generateSetAlgebra 𝒜 := .base
...
lemma generateSetAlgebra_induction {motive : ∀ s, s ∈ generateSetAlgebra 𝒜 → Prop}
  ...
  induction hs with
  | base _ hu => exact base _ hu
  | empty => exact empty
  | compl _ _ ih => exact compl _ _ ih
  | union _ _ _ _ ih₁ ih₂ => exact union _ _ _ _ ih₁ ih₂
