...
Algebras of sets are identified with Boolean subalgebras of the powerset algebra. The algebra
generated by a family is therefore defined through `BooleanSubalgebra.closure`, while this file
retains the set-specific finite normal form and countability results. We also prove that the
`σ`-algebra generated by a family of sets `𝒜` is the same as the one generated by its algebra.

## Main definitions

* `MeasureTheory.IsSetAlgebra`: property of being an algebra of sets.
* `MeasureTheory.IsSetAlgebra.toBooleanSubalgebra`: bundle an algebra of sets as a Boolean
  subalgebra of a powerset.
...
/-- An algebra of sets is a π-system. -/
theorem isPiSystem (h𝒜 : IsSetAlgebra 𝒜) : IsPiSystem 𝒜 :=
  h𝒜.isSetRing.isSetSemiring.isPiSystem
...
/-- Bundle an algebra of sets as a Boolean subalgebra of the powerset Boolean algebra. -/
@[expose] def toBooleanSubalgebra (h𝒜 : IsSetAlgebra 𝒜) : BooleanSubalgebra (Set α) :=
  BooleanSubalgebra.ofBotSupCompl 𝒜 h𝒜.empty_mem
    (fun _ hs _ ht ↦ h𝒜.union_mem hs ht) h𝒜.compl_mem

@[simp]
theorem coe_toBooleanSubalgebra (h𝒜 : IsSetAlgebra 𝒜) :
    (h𝒜.toBooleanSubalgebra : Set (Set α)) = 𝒜 := rfl
...
/-- A Boolean subalgebra of a powerset is an algebra of sets. -/
theorem isSetAlgebra (𝒜 : BooleanSubalgebra (Set α)) :
    MeasureTheory.IsSetAlgebra (𝒜 : Set (Set α)) where
  empty_mem := 𝒜.bot_mem
  compl_mem := by
    intro s hs
    exact 𝒜.compl_mem hs
  union_mem := by
    intro s t hs ht
    exact 𝒜.sup_mem hs ht

@[simp]
theorem toBooleanSubalgebra_isSetAlgebra (𝒜 : BooleanSubalgebra (Set α)) :
    MeasureTheory.IsSetAlgebra.toBooleanSubalgebra 𝒜.isSetAlgebra = 𝒜 := by
...
theorem isSetAlgebra (h𝒜 : IsSigmaAlgebra 𝒜) : MeasureTheory.IsSetAlgebra 𝒜 where
  empty_mem := h𝒜.empty_mem
  compl_mem := h𝒜.compl_mem
  union_mem := by
    intro s t hs ht
    exact h𝒜.union_mem hs ht
...
/-- View a sigma-algebra as a Boolean subalgebra of the powerset Boolean algebra. -/
@[expose] def toBooleanSubalgebra (𝓐 : SigmaAlgebra α) : BooleanSubalgebra (Set α) :=
  MeasureTheory.IsSetAlgebra.toBooleanSubalgebra 𝓐.isSigmaAlgebra.isSetAlgebra

@[simp]
theorem coe_toBooleanSubalgebra (𝓐 : SigmaAlgebra α) :
    (toBooleanSubalgebra 𝓐 : Set (Set α)) = 𝓐 := rfl

/-- The measurable sets of a sigma-algebra form an algebra of sets. -/
theorem isSetAlgebra (𝓐 : SigmaAlgebra α) :
    MeasureTheory.IsSetAlgebra (𝓐 : Set (Set α)) :=
  (toBooleanSubalgebra 𝓐).isSetAlgebra
...
@[expose] def generateSetAlgebra {α : Type*} (𝒜 : Set (Set α)) : Set (Set α) :=
  BooleanSubalgebra.closure 𝒜

@[simp]
theorem generateSetAlgebra_eq_booleanSubalgebra_closure :
    generateSetAlgebra 𝒜 = (BooleanSubalgebra.closure 𝒜 : Set (Set α)) := rfl

/-- The algebra of sets generated by a family of sets is an algebra of sets. -/
theorem isSetAlgebra_generateSetAlgebra :
    IsSetAlgebra (generateSetAlgebra 𝒜) :=
  BooleanSubalgebra.isSetAlgebra (BooleanSubalgebra.closure 𝒜)
...
/-- An induction principle for membership of `generateSetAlgebra 𝒜`. If `motive` holds of all
elements of `𝒜` and of `∅`, and is preserved under complement and binary union, then it holds of all
elements of `generateSetAlgebra 𝒜`. -/
@[elab_as_elim]
lemma generateSetAlgebra_induction {motive : ∀ s, s ∈ generateSetAlgebra 𝒜 → Prop}
    (base : ∀ (u) (hu : u ∈ 𝒜), motive u (self_subset_generateSetAlgebra hu))
    (empty : motive ∅ isSetAlgebra_generateSetAlgebra.empty_mem)
    (compl : ∀ u hu, motive u hu → motive uᶜ (isSetAlgebra_generateSetAlgebra.compl_mem hu))
    (union : ∀ u v hu hv, motive u hu → motive v hv →
      motive (u ∪ v) (isSetAlgebra_generateSetAlgebra.union_mem hu hv))
    (hs : s ∈ generateSetAlgebra 𝒜) : motive s hs := by
  induction hs using BooleanSubalgebra.closure_bot_sup_induction with
  | mem u hu => exact base u hu
  | bot => exact empty
  | sup u hu v hv ihu ihv => exact union u v hu hv ihu ihv
  | compl u hu ihu => exact compl u hu ihu

/-- `generateSetAlgebra 𝒜` is the smallest algebra of sets containing `𝒜`: if a family of sets `𝒜`
is contained in an algebra of sets `ℬ`, then so is the algebra of sets generated by `𝒜`. -/
lemma IsSetAlgebra.generateSetAlgebra_subset {ℬ : Set (Set α)} (h : 𝒜 ⊆ ℬ)
...
theorem generateFrom_generateSetAlgebra_eq :
  ...
  | basic t ht =>
    induction ht using generateSetAlgebra_induction with
    | base u u_mem => exact mem_generateFrom u_mem
    | empty => exact (generateFrom 𝒜).empty_mem
    | compl _ _ hu => exact (generateFrom 𝒜).compl_mem hu
    | union _ _ _ _ hu hv => exact (generateFrom 𝒜).union_mem hu hv
  | empty => exact (generateFrom 𝒜).empty_mem
  | compl _ _ ht => exact (generateFrom 𝒜).compl_mem ht
  | iUnion _ _ ht => exact (generateFrom 𝒜).iUnion_mem ht
...
theorem generateSetAlgebra_mono {ℬ : Set (Set α)} (h : 𝒜 ⊆ ℬ) :
  ...
  isSetAlgebra_generateSetAlgebra.generateSetAlgebra_subset (h.trans self_subset_generateSetAlgebra)
...
theorem generateSetAlgebra_subset_self (h𝒜 : IsSetAlgebra 𝒜) :
    generateSetAlgebra 𝒜 ⊆ 𝒜 := generateSetAlgebra_subset subset_rfl h𝒜
